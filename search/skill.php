...
{
	$sql = "SELECT job_skills.*, manage_job_skills.job_id, manage_job_skills.job_skill_id FROM job_skills LEFT JOIN manage_job_skills ON job_skills.id = manage_job_skills.job_skill_id WHERE job_skills.slug = :name AND job_skills.status = '1' GROUP BY job_skills.id";
    ...
    {
        $job_id = $rw->job_id;
        $query = "SELECT jobs.*, states.state, cities.city, functional_areas.functional_area, companies.companyname, companies.logo, industries.industry, job_types.job_type, job_experiences.job_experience FROM jobs LEFT JOIN states ON jobs.state_id = states.id LEFT JOIN cities ON jobs.city_id = cities.id LEFT JOIN functional_areas ON jobs.functional_area_id = functional_areas.id LEFT JOIN companies ON jobs.company_id = companies.id LEFT JOIN industries ON jobs.industry_id = industries.id LEFT JOIN job_types ON jobs.job_type_id = job_types.id LEFT JOIN job_experiences ON jobs.job_experience_id = job_experiences.id WHERE jobs.job_id = :job_id AND jobs.status = '1' AND jobs.verified = '1' AND (jobs.job_expiry_date > CURDATE())";
		$qry = $conn->prepare($query);
		$qry->bindParam(":job_id", $job_id, PDO::PARAM_INT);
		$qry->execute();
...
<script>
    $(document).ready(function() {
        ...
        function filter_data() {
            ...
            $.ajax({
                url: "fetch_skills_results.php?name=<?php echo $name; ?>",
                method: "POST",
                data: {
                    action: action,
                    states: states,
