					 ...
					 <div class="job-list-modern">
					 	<div class="job-listings-sec no-border">
						 	<div class="row">
							 	...
							 	{
							 		$retrieve = "SELECT * FROM companies WHERE status = 1 AND verified = 1 AND is_featured = 1 ORDER BY companyname";
									...
									if($qery->rowCount() > 0)
									{
										foreach($results as $fetch)
										{
											echo '
											<div class="col-md-3">
												<p class="fetch-list"><a href="'.$path.'/search/company?name='.$fetch->slug.'">'.$fetch->companyname.'</a></p>
											</div>';
										}
									}
							 	}
							 	...
							 	{
							 		$retrieve = "SELECT * FROM companies WHERE status = 1 AND verified = 1 ORDER BY companyname";
									$qery = $conn->prepare($retrieve);
									$qery->execute();
									$results=$qery->fetchAll(PDO::FETCH_OBJ);
									if($qery->rowCount() > 0)
