...
{
	$sql = "SELECT jobs.*, states.state, cities.city, functional_areas.functional_area, companies.companyname, companies.logo, industries.industry, job_types.job_type, job_experiences.job_experience, qualifications.qualification, qualifications.abbreviation FROM jobs LEFT JOIN states ON jobs.state_id = states.id LEFT JOIN cities ON jobs.city_id = cities.id LEFT JOIN functional_areas ON jobs.functional_area_id = functional_areas.id LEFT JOIN companies ON jobs.company_id = companies.id LEFT JOIN industries ON jobs.industry_id = industries.id LEFT JOIN job_types ON jobs.job_type_id = job_types.id LEFT JOIN job_experiences ON jobs.job_experience_id = job_experiences.id LEFT JOIN qualifications ON jobs.qualification_id = qualifications.id WHERE qualifications.slug = :name AND jobs.status = 1 AND jobs.verified = 1 AND (jobs.job_expiry_date > CURDATE())";
