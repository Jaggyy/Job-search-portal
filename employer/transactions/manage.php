...
<head>
    ...
    <title>Employer Transactions | Naukri</title>
	<!-- Vendors Style-->
								...
								<div class="box-body">
									<div class="table-responsive">
						  				<table id="example1" class="table table-bordered table-striped">
											...
											$sql_retrieve = "SELECT * FROM revenue WHERE company_id = :company_id";
						                    ...
						                    if($query->rowCount() > 0)
						                    {
						                        foreach($results as $query_fetch)
						                        {
						                        	echo '
													<tr>
														<td>'.$query_fetch->created_at.'</td>
														<td>'.$query_fetch->amount.'</td>
														<td class="text-center">
															<a href="'.$path.'/employer/transactions/invoice?id='.$query_fetch->id.'" target="_blank" title="View Invoice"><span class="badge badge-lg badge-success"><i class="mdi mdi-eye"></i></span></a>
														</td>
													</tr>';
												}
											}
