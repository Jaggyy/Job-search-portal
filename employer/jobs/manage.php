...
if(isset($_GET['del']))
{
	...
	if($delete->execute())
	{
		...
		header('Refresh:2;url='.$path.'/employer/jobs/manage');
	}
	else
	{
				...
				<div class="content-header">
					<div class="d-flex align-items-center">
						...
						<div class="col-auto text-end float-end ml-auto">
							<a href="<?php echo $path; ?>/employer/jobs/create" class="btn fix-gr-bg btn-rounded"><i class="mdi mdi-plus"></i>&nbsp;&nbsp;Add New Job</a>
						</div>
					</div>
				</div>
				<!-- Main content -->
								...
								<div class="box-body">
									<div class="table-responsive">
						  				<table id="example1" class="table table-bordered table-striped">
											...
											$sql_retrieve = "SELECT jobs.*, cities.city AS city, states.state AS state FROM jobs LEFT JOIN states ON jobs.state_id = states.id LEFT JOIN cities ON jobs.city_id = cities.id WHERE jobs.company_id = :company_id";
						                    ...
						                    if($query->rowCount() > 0)
						                    {
						                        ...
						                        {
						                        	$job_id = $query_fetch->job_id;
						                        	$job_expiry_date = strtotime($query_fetch->job_expiry_date);
						                        	$currentDate = time();
						                        	echo '
													<tr>
														<td style="width: 30%;"><a href="'.$path.'/jobs?job='.$query_fetch->slug.'">'.$query_fetch->job_title.'</a></td>
														<td>'.date('jS M, Y', $job_expiry_date).'</td>';
														$count = "SELECT id FROM job_applied WHERE job_id = :job_id";
														$fetch = $conn->prepare($count);
														$fetch->bindParam(":job_id", $job_id, PDO::PARAM_INT);
														$fetch->execute();
														$cnt_applications = $fetch->rowCount();
														echo '
														<td class="text-center"><span class="badge badge-pill badge-primary">'.$cnt_applications.'</span></td>
														<td>';
														if($query_fetch->status == 1 && $query_fetch->is_featured == 0 && ($job_expiry_date > $currentDate))
														{
															echo '
															<a href="'.$path.'/employer/jobs/payment?id='.$query_fetch->id.'"><span class="badge badge-lg badge-primary">Make Featured</span></a>';
														}
														elseif($query_fetch->is_featured == 1)
														{
															echo '<span class="badge badge-lg badge-success">Featured</span>';
														}
														else
														{
															echo '<span class="badge badge-lg badge-warning">Not Featured</span>';
														}
														echo '</td>
														<td>';
														if($query_fetch->status == null)
														{
															echo '
															<span class="badge badge-lg badge-warning">Pending</span>';
														}
														elseif($job_expiry_date < $currentDate)
														{
															echo '
															<span class="badge badge-lg badge-danger">Closed</span>';
														}
														else
														{
															echo '
															<span class="badge badge-lg badge-success" style="padding: 3px 18px;">Live</span>';
														}
														echo '
														</td>
														<td>
															<div class="btn-group">
															  	...
															  	<div class="dropdown-menu dropdown-menu-end" style="margin: 0px;">
																	<a class="dropdown-item" href="'.$path.'/employer/jobs/applications?job_id='.$query_fetch->job_id.'">JOB APPLICATIONS</a>
																	<a class="dropdown-item" href="'.$path.'/employer/jobs/edit?id='.$query_fetch->id.'">EDIT</a>
																	<a class="dropdown-item" href="#delete'.$query_fetch->id.'" data-toggle="modal" data-target="#deleteJob'.$query_fetch->id.'">DELETE</a>
															  	</div>
															</div>
														</td>
													</tr>
													<!--Modal-->
													<div class="modal fade" id="deleteJob'.$query_fetch->id.'" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
													aria-hidden="true">
														<div class="modal-dialog modal-dialog-centered" role="document">
															<div class="modal-content">
																...
																<div class="modal-footer">
																	...
																	<a class="btn fix-gr-bg" href="'.$path.'/employer/jobs/manage?id='.$query_fetch->id.'&del=delete">Delete</a>
														  		</div>
															</div>
														</div>
													</div>';
