...
if(isset($_POST['submit']))
{
    ...
    $sql = "UPDATE jobs SET status = :status, updated_at = :updated_at WHERE id = :id";
...
if(isset($_POST['verify']))
{
    ...
    $sql = "UPDATE jobs SET status = :status, verified = :verified WHERE id = :id";
    $query = $conn->prepare($sql);
    $query->bindParam(':status', $status, PDO::PARAM_INT);
    ...
    if($result)
    {
        $success = "Job Verified Successfully.";
        header('Refresh:2;url='.$path.'/admin/jobs/view?id='.$_GET['id'].'');
    }
    else
    {
