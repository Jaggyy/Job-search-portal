...
if(isset($_GET['del']))
{
	...
	if($delete->execute())
	{
		...
		header('Refresh:2;url='.$path.'/admin/jobs/live');
	}
	else
	{
								...
								<div class="box-body">
									<div class="table-responsive">
						  				<table id="example1" class="table table-bordered table-striped">
											...
											<tbody>
											<?php
											$sql_retrieve = "SELECT * FROM jobs WHERE status = 1 AND verified = 1 AND (job_expiry_date > CURDATE()) ORDER BY job_title";
						                    ...
						                    if($query->rowCount() > 0)
						                    {
						                        ...
						                        {
						                        	$created_at = strtotime($query_fetch->created_at);
						                        	$job_expiry_date = strtotime($query_fetch->job_expiry_date);
						                        	echo '
													<tr>
														<td style="width: 30%;">'.$query_fetch->job_title.'</td>
														<td>'.date('jS M, Y', $created_at).'</td>
														<td><span class="text-danger">'.date('jS M, Y', $job_expiry_date).'</span></td>
														<td>';
														if($query_fetch->is_featured == null || $query_fetch->is_featured == 0)
														{
															echo '
															<span class="badge badge-lg badge-primary">Not Featured</span>';
														}
														else
														{
															echo '
															<span class="badge badge-lg badge-success">Featured</span>';
														}
														echo '
														</td>
														<td>
															<div class="btn-group">
															  	...
															  	<div class="dropdown-menu dropdown-menu-end" style="margin: 0px;">
																	<a class="dropdown-item" href="'.$path.'/admin/jobs/view?id='.$query_fetch->id.'">VIEW</a>
													...
													<!--Modal-->
													<div class="modal fade" id="deleteState'.$query_fetch->id.'" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
													aria-hidden="true">
														<div class="modal-dialog modal-dialog-centered" role="document">
															<div class="modal-content">
																...
																<div class="modal-footer">
																	...
																	<a class="btn fix-gr-bg" href="'.$path.'/admin/jobs/live?id='.$query_fetch->id.'&del=delete">Delete</a>
														  		</div>
															</div>
														</div>
													</div>';
